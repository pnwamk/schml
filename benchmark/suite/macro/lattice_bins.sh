...
run_benchmark()
{
    ...
    local static_source_file="${benchmarks_path}/${name}.schml"
    local dyn_source_file="${TMP_DIR}/dyn/${name}.schml"
    ...
    rm -rf "$lattice_path"
    rm -f "${lattice_path}.schml"
    cp "$static_source_file" "${lattice_path}.schml"
	local dynamizer_out=$(dynamizer "${lattice_path}.schml" "$nsamples" "$nbins" | sed -n 's/.* \([0-9]\+\) .* \([0-9]\+\) .*/\1 \2/p')
	echo "$dynamizer_out" > "$lattice_file"
    fi

    racket "${SCHML_DIR}/benchmark/benchmark.rkt" -s "$c1 $c2" "${lattice_path}/"
